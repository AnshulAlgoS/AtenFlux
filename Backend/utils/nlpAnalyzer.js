...
/**
 * Calculate journalist influence based on articles, topics, social presence, and role
 */
export function calculateInfluence(journalist) {
  let score = 50;  // Base score
  
  // Article count contribution (max +50)
  const articleCount = journalist.articles || journalist.totalArticles || 0;
  score += Math.min(articleCount * 2, 50);
  
  // Topic diversity contribution (max +15)
  const topicCount = journalist.topics?.length || 0;
  score += Math.min(topicCount * 3, 15);
  
  // Social media presence contribution (max +25)
  const socialLinks = journalist.socialLinks || {};
  if (socialLinks.twitter) score += 8;   // Twitter is most valuable for journalists
  if (socialLinks.linkedin) score += 7;  // LinkedIn is professional credibility
  if (socialLinks.facebook) score += 4;
  if (socialLinks.instagram) score += 3;
  if (socialLinks.youtube) score += 3;
  
  // Email available contribution (+5)
  const email = journalist.email || socialLinks.email;
  if (email) score += 5;
  
  // Bio quality contribution (max +10)
  if (journalist.bio && journalist.bio.length > 50) {
    score += Math.min(Math.floor(journalist.bio.length / 50), 10);
  }
  
  // Profile picture contribution (+5)
  ...
  if (role.includes('editor-in-chief') || role.includes('chief editor')) {
    score += 20;
  } else if (role.includes('managing editor') || role.includes('executive editor')) {
    score += 18;
  } else if (role.includes('bureau chief') || role.includes('resident editor')) {
    score += 15;
  } else if (role.includes('senior') || role.includes('principal') || role.includes('chief')) {
    score += 12;
  } else if (role.includes('special') || role.includes('columnist')) {
    score += 10;
  } else if (role.includes('editor')) {
    score += 8;
  } else if (role.includes('correspondent') || role.includes('reporter')) {
    score += 5;
  }
  
  // Cap at 150 for exceptional journalists
  return Math.min(parseFloat(score.toFixed(1)), 150);
}
